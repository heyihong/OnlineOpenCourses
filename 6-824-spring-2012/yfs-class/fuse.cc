...
#if 1
	std::string buf;
	if (yfs->getbuf(ino, 0, attr->st_size, buf) != yfs_client::OK) {
		fuse_reply_err(req, ENOENT);
		return;
	}
	buf.resize(attr->st_size);
	if (yfs->setbuf(ino, 0, buf, true) != yfs_client::OK) {
		fuse_reply_err(req, ENOENT);
		return;
	}
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  if (yfs->getbuf(ino, off, size, buf) != yfs_client::OK) {
  	fuse_reply_err(req, ENOENT);
	return; 
  }
                 ...
                 struct fuse_file_info *fi)
{
  ...
  if (yfs->setbuf(ino, off, str, false) != yfs_client::OK) {
	fuse_reply_err(req, ENOENT);	
	return;
  }
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  dr.name = std::string(name);
  dr.inum = (random() &0x7fffffff) | e->ino;
  ret = yfs->create(parent, dr);
  if (ret != yfs_client::OK) {
	  return ret;
  }
  e->ino = dr.inum;
  ret = getattr(dr.inum, e->attr);
  return ret;
}
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  std::vector<yfs_client::dirent> dirents;
  std::vector<yfs_client::dirent>::iterator iter;
  if (yfs->getdirents(ino, dirents) != yfs_client::OK) {
    fuse_reply_err(req, ENOENT); 
    return;
  }
  for (iter = dirents.begin(); iter != dirents.end(); iter++) {
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  if (yfs->rmfile(parent, std::string(name)) == yfs_client::OK) {
