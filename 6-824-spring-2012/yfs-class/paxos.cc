         ...
         std::string &v)
{
  ...
  for (size_t i = 0; i < nodes.size(); i++) {
		...
		rpcc *cl = h.safebind();
		if (cl) {
			ret = cl->call(paxos_protocol::preparereq, me, args, res, rpcc::to(1000));
			if (ret == paxos_protocol::OK) {
				if (res.oldinstance) {
				  acc->commit(instance, res.v_a);
				  return false;
				} else if (res.accept) {
					if (accepts.empty() || res.n_a > max_n_a) {
						max_n_a = res.n_a;
						v = res.v_a;
					}	
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for (size_t i = 0; i < nodes.size(); i++) {
		...
		rpcc *cl = h.safebind();
		if (cl) {
			ret = cl->call(paxos_protocol::acceptreq, me, args, r, rpcc::to(1000));
	      ...
	      std::string v)
{
  ...
  for (size_t i = 0; i < accepts.size(); i++) {
	  ...
	  rpcc *cl = h.safebind();
		if (cl) {
      cl->call(paxos_protocol::decidereq, me, args, r, rpcc::to(1000));
		}
  }
}
    ...
    paxos_protocol::prepareres &r)
{
  ...
  if (a.instance <= instance_h) {
		...
		r.oldinstance = false;
		if (a.instance == instance_h + 1 && a.n > n_h) {
			r.accept = true;
			n_h = a.n; 
			r.n_a = n_a; 
			r.v_a = v_a;
...
acceptor::acceptreq(std::string src, paxos_protocol::acceptarg a, bool &r)
{
  ...
  ScopedLock ml(&pxs_mutex); 
  r = a.instance == instance_h + 1 && a.n >= n_h;
  if (r) {
		n_h = a.n;
		n_a = a.n;
		v_a = a.v;
