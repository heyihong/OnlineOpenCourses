...
rsm::sync_with_backups()
{
  ...
  //   - or there is a committed viewchange
	...
	for (std::vector<std::string>::iterator iter = backups.begin(); iter != backups.end(); iter++) {
		if (*iter == primary) {
			backups.erase(iter);
			break;
		}
	}		
	while (!backups.empty() && vid_insync == vid_commit) {
		pthread_cond_wait(&sync_cond, &rsm_mutex);
	}
  insync = false;
  return vid_insync == vid_commit;
}
...
rsm::sync_with_primary()
{
  ...
  // or there is a commited viewchange
	...
	while (vid_insync == vid_commit) {
	 ...
	 } else if (statetransferdone(primary)) {
		 break;
	 }
	}
  return vid_insync == vid_commit;
...
rsm::statetransferdone(std::string m) {
  ...
  // - Inform primary that this slave has synchronized for vid_insync
	...
	handle h(m);
	int ret;
	tprintf("rsm::statetransferdone: contact %s w. my last_myvs_vid(%d)\n",
  ...
  tprintf("rsm::statetransferdone transferdone to %s success, vs_vid(%d)\n", 
	 m.c_str(), last_myvs.vid);
  return true;
}
...
{
	tprintf("client_invoke(%d, req_size = %lu)\n", procno, req.size());
  ...
  // You fill this in for Lab 7
	...
	if (inviewchange) {
		...
		viewstamp vs = myvs;
		myvs.seqno++;
		std::vector<std::string> mems = cfg->get_view(vs.vid);	
		pthread_mutex_unlock(&rsm_mutex);	
		{
			ScopedLock l(&invoke_mutex);
			int t;
			int rret = rsm_client_protocol::OK;
			for (size_t i = 0; i < mems.size(); i++) {
				if (mems[i] != cfg->myaddr()) {
					...
					bool flag = true;
					while (flag) {
						rret = cl->call(rsm_protocol::invoke, procno, vs, req, t, rpcc::to(1000));
						if (rret == rsm_protocol::OK) {
							break;
						}
						pthread_mutex_lock(&rsm_mutex);
						flag = vs.vid == vid_commit;
						pthread_mutex_unlock(&rsm_mutex);
					}
					if (rret != rsm_protocol::OK) {
						ret = rsm_client_protocol::BUSY;
						...
						// code in this block only used for tests
						pthread_mutex_lock(&rsm_mutex);
						breakpoint1();
						partition1();	
						pthread_mutex_unlock(&rsm_mutex);
...
{
	tprintf("invoke(%d, {vid=%u, seqno=%u}, req.size() = %lu)\n", proc, vs.vid, vs.seqno, req.size());
  rsm_protocol::status ret = rsm_protocol::OK;
  // You fill this in for Lab 7
	ScopedLock ml(&invoke_mutex);
	ScopedLock l(&rsm_mutex);
	if (last_myvs.vid < vs.vid || (last_myvs.vid == vs.vid && last_myvs.seqno + 1 == vs.seqno)) {
		pthread_mutex_unlock(&rsm_mutex);
		std::string r;
		execute(proc, req, r);
		breakpoint1();
...
rsm::transferdonereq(std::string m, unsigned vid, int &)
{
  ...
  // - Wake up recovery thread if all backups are synchronized
	tprintf("transferdone from %s vid (%u)\n", m.c_str(), vid);
	if (!insync || vid != vid_insync) {
		return rsm_protocol::BUSY;
	}
	for (std::vector<std::string>::iterator iter = backups.begin(); iter != backups.end(); iter++) {
