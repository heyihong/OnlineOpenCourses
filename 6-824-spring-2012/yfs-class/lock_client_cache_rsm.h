...
class lock_client_cache_rsm : public lock_client {
 private:
  ...
  std::string id;
	pthread_t th;
	lock_protocol::xid_t acq_xid; 
  lock_protocol::xid_t rel_xid;
	lock_protocol::xid_t reply;
	enum lock_status {
	  NONE = 0,
		FREE,
		LOCKED,
	...
	fifo<lock_protocol::lockid_t> releases;
	std::map<lock_protocol::lockid_t, lockent> locks;
	pthread_cond_t hdl_cond;
	pthread_mutex_t mu;
 public:
