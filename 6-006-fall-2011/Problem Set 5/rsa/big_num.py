...
class BigNum(object):
  ...
  def slow_mul(self, other):
    ...
    for i in range(len(self.d)):
        ...
        for j in range(len(other.d)):
            word = result.d[i + j].word() + self.d[i] * other.d[j] + carry.word()
            carry = word.msb()
  ...
  def slow_divmod(self, other):
    ...
    for i in range(1, 8):
        (num, p) = s[i - 1]
        s.append((num + num, (p + p).lsb()))

    quotient = BigNum.zero(len(self.d))
    remainder = BigNum.zero()
    for i in range(len(self.d) - 1, -1, -1):
        remainder = remainder << 1 
        remainder.d[0] = self.d[i]
        byte = Byte.zero() 
