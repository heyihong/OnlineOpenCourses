...
import avl    # Used to fasten RangeIndex
...
class RangeIndex(object):
  ...
  def list(self, first_key, last_key):
    ...
    result = []
    node = self.data.lower_bound(first_key)
    while node is not None and node.key <= last_key:
        result.append(node.key)
        node = self.data.successor(node) 
    return result
  ...
  def _compute_crossings(self, count_only):
    ...
    for event in self.events:
      ...
      if event_type == 'add':
        ...
        self.trace_sweep_line(event_x)
        pairL = KeyWirePairL(wire.y1)
        pairH = KeyWirePairH(wire.y2)
        if count_only:
          result += self.index.count(pairL, pairH)
        else:
          for kwp in self.index.list(pairL, pairH):
