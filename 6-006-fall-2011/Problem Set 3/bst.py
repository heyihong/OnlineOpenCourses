...
"""
    ...
    def lower_bound(self, t):
        """Return the node for t <= key in this tree, or None otherwise.""" 
        if self.key < t:
            if self.right is None:
                return None
            return self.right.lower_bound(t)
        # self.key >= t
        temp = None if self.left is None else self.left.lower_bound(t) 
        return self if temp is None else temp
...
"""
    ...
    def lower_bound(self, t):
        """Return the node for t <= key if is in the tree, or None otherwise."""
        if self.root is None:
            return None
        else:
            return self.root.lower_bound(t)
    ...
    def successor(self, node):
        """Returns the node with the smallest key larger than this node's key, or None if this has the largest key in the tree."""
        node = node.successor()
        return node if node != self.psroot else None
