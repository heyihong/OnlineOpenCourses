...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            node.visited = False 
        pq = PriorityQueue()
        pairs = {source : NodeDistancePair(source, 0)}
        pq.insert(pairs[source])
        num_visited = 0
        while True: 
            min_pair = pq.extract_min()
            (node, dist) = (min_pair.node, min_pair.distance)
            node.visited = True 
            num_visited += 1
            if node == destination:
                break
            for adj in node.adj:
                new_dist = dist + weight(node, adj)
                if not adj.visited:
                    if adj in pairs:
                        pair = pairs[adj]
                        pair.distance = min(pair.distance, new_dist)
                        pq.decrease_key(pair)
                    else:
                        pairs[adj] = NodeDistancePair(adj, new_dist)
                        pq.insert(pairs[adj])
        path = []
        node = destination
        while node != source:
            path.append(node)
            for adj in node.adj:
                if adj.visited and pairs[adj].distance + weight(adj, node) == pairs[node].distance:
                    node = adj
                    break 
        path.append(node)
        path.reverse()
